...
class BaseDataset(data.Dataset):
    ...
    def read_object_grasp_data(
        self, filename, quality='quality_flex_object_in_gripper', 
        ...
        if filename.endswith(".json"):
            ...
            mesh_scale = data["object_scale"]
            grasps = np.array(data["transforms"]) 
            flex_qualities = np.array(data[quality])
        elif filename.endswith(".h5"):
            quality = []
            data = h5py.File(filename, "r")
            mesh_fname = data["object/file"][()].decode('utf-8')
            mesh_scale = data["object/scale"][()] 
            #grasps = np.array(data["grasps/transforms_adjust"])  #grasps/transforms_adjust
            #flex_qualities = np.array(data['grasps/qualities/flex/object_in_gripper'])
            
            #single affordance grasp
            grasps = np.array(data["grasps/good_grasp"])
            if len(grasps)==0:
                grasps = np.zeros((1,4,4))
                grasps[:,3,3]=1
                for i in range(len(grasps)):     #single affordance grasp
                    quality.append(0)
                flex_qualities = np.array(quality)  #single affordance grasp
            else:
                for i in range(len(grasps)):     #single affordance grasp
                    quality.append(1)
                flex_qualities = np.array(quality)  #single affordance grasp
        ...
        object_model = Object(os.path.join(root_folder,mesh_fname))
        ...
        try:
            heuristic_qualities = np.asarray(data['quality_number_of_contacts'])
        except KeyError:
        ...
        return positive_grasps, positive_qualities, negative_grasps, negative_qualities, object_model, os.path.join(root_folder, mesh_fname), mesh_scale
