...
import h5py
import json
from utils2.provider import rotate_point_cloud_SO3, rotate_point_cloud_y
...
class AffordNetDataset(Dataset):
    ...
    def load_data(self):
        self.all_data = []
        print('w/o partial and split now',self.partial,self.split)
        #
        if self.semi:
        ...
        for index, info in enumerate(temp_data):
            if self.partial and self.split == 'train':
                ...
                for r, r_data in rotate_info.items():
                    ...
                    temp_info["semantic class"] = info["semantic class"]
                    temp_info["affordance"] = info["affordance"]
                    temp_info["data_info"] = partial_info
    ...
    def __getitem__(self, index):
        ...
        if self.split != 'train' and self.rotate != 'None':
            data_info = data_info['view_1']    ###test at a special view
