...
class GraspNetModel:
    ...
    def backward(self, out):
        if self.opt.arch == 'vae':
            ...
            predicted_cp, confidence = out  #torch.Size([24, 7]) torch.Size([24])
            predicted_cp = utils.transform_control_points(
                predicted_cp, predicted_cp.shape[0], device=self.device) ##torch.Size([24, 6, 3])
...
##################
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            print('hhhhh',prediction.shape)
            if self.opt.arch == "vae":
                print('hh',prediction.shape)
                predicted_cp = utils.transform_control_points(
                    prediction, prediction.shape[0], device=self.device)
                print('jjjjj',predicted_cp.shape,self.targets.shape)
                ...
                predicted_cp = utils.transform_control_points(
                    prediction, prediction.shape[0], device=self.device)
                print('jjjjj',predicted_cp.shape)
                reconstruction_loss, _ = self.criterion(
                    ...
                    confidence_weight=self.opt.confidence_weight,
                    device=self.device)
                #print('KKKKKK',reconstruction_loss)
                return reconstruction_loss, 1
            else:
