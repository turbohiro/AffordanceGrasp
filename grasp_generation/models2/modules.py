...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k=None, v=None, attn_mask=None, return_attn=False):
        ...
        mb_size, len_q, d_mode = q.size()
        mb_size, len_k, d_model = k.size()
        mb_size, len_v, d_model = v.size()
        #print(mb_size,d_model,len_q) 8 2048 128
  
        # treat as a (n_head) size batch
        q_s = q.repeat(n_head, 1, 1).view(n_head, -1, d_model) # n_head x (mb_size*len_q) x d_model 8*(8*128)*2048
        k_s = k.repeat(n_head, 1, 1).view(n_head, -1, d_model) # n_head x (mb_size*len_k) x d_model
        v_s = v.repeat(n_head, 1, 1).view(n_head, -1, d_model) # n_head x (mb_size*len_v) x d_model

        # treat the result as a (n_head * mb_size) size batch
        #print("aqw",q_s.shape,self.w_qs.shape,k_s.shape,self.w_ks.shape)
        #pdb.set_trace()
