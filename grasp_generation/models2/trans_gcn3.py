...
class PointGenCon(nn.Module):
    def __init__(self,  bottleneck_size = 8192):
        ...
        #self.conv1 = nn.Sequential(nn.Conv1d(257, 256, kernel_size=1, bias=False),self.bn1, nn.LeakyReLU(negative_slope=0.2))
        self.conv2 = nn.Sequential(nn.Conv1d(193, 128, kernel_size=1, bias=False),self.bn2, nn.LeakyReLU(negative_slope=0.2))
...
class TransGCN_Estimation(nn.Module):
    def __init__(self, args, num_classes, bottleneck_size = 256, use_attention = True):
        ...
        self.dropout  = nn.Dropout(p=0.1)
        self.mha_1 = MultiHeadAttention(n_head = 4,d_model =512)
    ...
    def forward(self, x):
       ...
       # x = self.dropout(x)
        ...
        #x = self.ffn_1(x)
        #x = self.dropout(x)
        #x2_att = x.transpose(2, 1).contiguous()
       ...
       # x5 = self.mha_1(x5)
        ...
        # (batch_size, 1088+64*3, num_points)
            xt = torch.cat((x0,x_att,x), dim=1) #512 256 64
        ...
        for index, classifier in enumerate(self.classifier):
            ...
            #rand_value.data.uniform_(0,1)
            #y = x.unsqueeze(2).expand(x.size(0),x.size(1), rand_value.size(2)).contiguous()
            #y = torch.cat((rand_value, y), 1).contiguous()
            #pdb.set_trace()
            score_ = classifier(y)
