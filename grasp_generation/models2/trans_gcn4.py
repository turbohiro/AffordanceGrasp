...
class PointGenCon(nn.Module):
    def __init__(self,  bottleneck_size = 8192):
        ...
        self.conv1 = nn.Sequential(nn.Conv1d(1216, 256, kernel_size=1, bias=False),self.bn1, nn.LeakyReLU(negative_slope=0.2))
        ...
        self.conv4 = torch.nn.Conv1d(self.bottleneck_size//4, 9, 1)
...
class TransGCN_Estimation(nn.Module):
    def __init__(self, args, num_classes,bottleneck_size = 256):
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(64*2, 64, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv6 = nn.Sequential(nn.Conv1d(64*4, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(16, 64, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(768, 256, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x):
        ...
        out = []
        for i in range(0,16):
            rand_value = Variable(torch.cuda.FloatTensor(x.size(0),2,num_points/4))
        ...
        # (batch_size, 1088+64*3, num_points) -> (batch_size, 256, num_points)
            out.append(self.classifier[0](y))
        out = torch.cat(out,2).contiguous() 
