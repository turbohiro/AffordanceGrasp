...
class PointGenCon(nn.Module):
    def __init__(self,  bottleneck_size = 8192):
        ...
        self.conv1 = nn.Sequential(nn.Conv1d(1026, 256, kernel_size=1, bias=False),self.bn1, nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x):
       ...
       # x5 = self.mha_1(x5)
        ...
        # (batch_size, 1088+64*3, num_points)
            xt = torch.cat((x, x0, x1,x2_tran,x3), dim=1) #512 256 64
        ...
        #x = self.nn(x)
        #pdb.set_trace()
        rand_value = Variable(torch.cuda.FloatTensor(x.size(0),2,num_points))
