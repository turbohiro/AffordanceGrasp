...
def nonuniform_quaternions():
    ...
    if train:
        ...
        all_poses = []
        for az in np.linspace(-np.pi/2, -np.pi/3 , 10):
            for el in np.linspace(-np.pi / 3, np.pi / 3, 10):  #grasp
                all_poses.append(tra.euler_matrix(el, az, 0))
        return all_poses
    elif test:
        all_poses = []
        for az in np.linspace(-np.pi/2, -np.pi/3 , 15):
            for el in np.linspace(-np.pi / 2, -np.pi / 3, 15):  #all affordance
                #R1 = tra.euler_matrix(0, -np.pi/2, 0)
                R2 = tra.euler_matrix(-np.pi/2 ,0 , 0)
                #R3 = tra.euler_matrix(0, 0, -np.pi/2)
                #R = np.dot(R3,R2)
                #all_poses.append(tra.euler_matrix(0, -np.pi/4 , 0))   
                all_poses.append(R2) 
        return all_poses
...
def transform_control_points(gt_grasps, batch_size, mode='qt', device=None):
    """
    ...
    if mode == 'qt':
        ...
        #print('hghg',gt_control_points.shape)
        ...
        #print('hhh',control_points.shape,ones.shape)
