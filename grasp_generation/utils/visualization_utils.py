...
def draw_scene(data,
    ...
    pc = data["pc"][0]
    print(pc.shape)
    grasp_gt = data['grasp_rt'].reshape((-1,4,4))
    #pc = data
    max_grasps = 500
    ...
    if len(grasps) > max_grasps:
        ...
        if grasp_scores is not None:
            ...
            affordance_score = np.array(affordance_score)[chosen_ones]
    ...
    #connect with control_points model (npy) file with gripper
    grasp_pc = np.squeeze(utils.get_control_point_tensor(1, False), 0)  #(6,3)
    grasp_pc[2, 2] = 0.059
    grasp_pc[3, 2] = 0.059
    ...
    grasp_pc = np.asarray(modified_grasp_pc) #(6*3)
    ...
    if grasp_scores is not None and affordance_score is not None:   #if affordance score is included
        grasp_scores = (np.asarray(grasp_scores) + np.asarray(affordance_score))/2
        indexes_visual = np.argsort(-grasp_scores)
    else:
    ...
    for ii in range(20):
        ...
        if show_gripper_mesh:
            ...
            #print('selected', i, grasp_scores[i], pts.shape)
            mid_points.append([pts[:, 0][2],pts[:, 1][2],pts[:, 2][2]])
            ...
            if isinstance(gripper_color, list):
                ...
                #gt = grasp_gt[i]
                #pts_gt = transform_grasp_pc(gt)
               
                g_visual = grasps[i_visual]
                pts_visual = transform_grasp_pc(g_visual)
                ...
                mlab.plot3d(pts[:, 0],
                            ...
                            opacity=1)
                mlab.plot3d(pts_visual[:, 0],
                            pts_visual[:, 1],
                            pts_visual[:, 2],
                            color=(0.5,0,1),
                            tube_radius=tube_radius,
                            opacity=1)
                #mlab.plot3d(pts_gt[:, 0],
                #            pts_gt[:, 1],
                #            pts_gt[:, 2],
                #            color=(1,0,0),
                #            tube_radius=tube_radius,
